...
NTSTATUS LoadLibraryFromMem(_In_ PVOID pvImage, _In_opt_ ULONG_PTR Size, _Out_opt_ void** ppv)
{
	NTSTATUS status;
...
#error not implemented !
		...
		if (0 <= (status = RtlAppendUnicodeToString(&ObjectName, USER_SHARED_DATA->NtSystemRoot)) &&
			0 <= (status = RtlAppendUnicodeToString(&ObjectName, _YW(L"\\system32\\"))) &&
...
void __fastcall ep(_In_ PVOID pv, _In_ ULONG cb)
{
	CPP_FUNCTION;

	WCHAR msg[0x100], *psz;
	NTSTATUS status = Unzip(pv, cb, &pv, &cb);

	if (NOERROR == status)
	{
		PVOID hmod;
		status = LoadLibraryFromMem(pv, cb, &hmod);
		LocalFree(pv);

		if (0 > status)
		{
			if (cb = FormatMessageW(FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_IGNORE_INSERTS,
				GetNtBase(), status, 0, msg, _countof(msg), 0))
			{
				cb++;
				swprintf_s(psz = msg + cb, _countof(msg) - cb, L"Load = %x", status);
				MessageBoxW(0, msg, psz, MB_ICONWARNING);
			}
		}
		else
		{
			swprintf_s(msg, _countof(msg), _YW(L"DLL loaded at %p"), hmod);
			MessageBoxW(0, msg, _YW(L"Load Ok !"), MB_ICONINFORMATION);
			FreeLibrary((HMODULE)hmod);
		}
	}
	else
	{
		if (cb = FormatMessageW(FORMAT_MESSAGE_IGNORE_INSERTS| FORMAT_MESSAGE_FROM_SYSTEM,
			0, status, 0, msg, _countof(msg), 0))
		{
			cb++;
			swprintf_s(psz = msg + cb, _countof(msg) - cb, L"Unzip = %x", status);
			MessageBoxW(0, msg, psz, MB_ICONWARNING);
		}
	}
